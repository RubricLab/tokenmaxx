...
// Quotanaut runs on the Bun runtime (bun:sqlite, Bun.TOML, Bun.spawn, …), but it
...
if (!bun) {
	process.stderr.write(
		'quotanaut runs on the Bun runtime, which was not found on this system.\n' +
			'Install Bun, then run quotanaut again:\n' +
...
if (result.error) {
	process.stderr.write(`quotanaut: could not start Bun (${result.error.message})\n`)
	process.exit(1)
}
