...
describe('prepareCodexAuthImport', () => {
	test('prepares a normalized account and credential without writing a vault', () => {
		const prepared = prepareCodexAuthImport(credential)
		...
		expect(prepared.account.secretReference).toBe(`codex:${prepared.account.id}`)
		expect(JSON.parse(prepared.serializedAuth)).toEqual(JSON.parse(credential))
	})

	test.each([
		['malformed json', '{"tokens":'],
		['missing token fields', '{"tokens":{}}'],
		[
			'missing verified email',
			JSON.stringify({
				tokens: {
					access_token: fakeJwt({ chatgpt_account_id: 'acct-secret' }),
					account_id: 'acct-secret',
					id_token: fakeJwt({ chatgpt_account_id: 'acct-secret' }),
					refresh_token: 'refresh-secret'
				}
			})
		]
	])('maps %s to one generic non-secret error', (_scenario, serialized) => {
		let caught: unknown
		try {
			prepareCodexAuthImport(serialized)
		} catch (error) {
			caught = error
		}
		...
		expect(caught).toMatchObject({
			...
			message: 'Codex auth data is invalid or does not contain a verified account identity'
		})
		expect(String(caught)).not.toContain('acct-secret')
