...
describe('selectRotation', () => {
	...
	test('bare hard limit without window headers rotates at full pressure', () => {
		const limited = { ...usage(1, 0, { hardLimitReached: true }), windows: [] }
		const decision = selectRotation({
			accounts: [account(1), account(2)],
			now: NOW,
			state: state(1, { switchedAgoMs: 60_000 }),
			usage: [limited, usage(2, 10)]
		})
		expect(decision).toMatchObject({
			reason: 'hardLimit',
			rotate: true,
			sourcePressure: 100,
			targetAccountId: account(2).id
		})
	})
