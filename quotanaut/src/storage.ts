...
function migratePolicyDefaults(database: Database): void {
	const rows = database.query<JsonRow & { provider: string }, []>(
		"SELECT provider, payload FROM provider_states WHERE provider = 'openai'"
	)
  ...
  `)
	...
	if (
		...
		const migrationRows = database
			.query<AccountMigrationRow, []>("SELECT id, payload FROM accounts WHERE provider = 'openai'")
			.all()
	...
	insertState.run('openai', serialize(initialProviderState('openai')))
...
export function createStateStore(databasePath: string): StateStore {
	...
	function listAccounts(provider?: ProviderId): Account[] {
		return provider
			...
			: queryAll(
					AccountSchema,
					"SELECT payload FROM accounts WHERE provider = 'openai' ORDER BY label, id"
				)
	}

	function findAccount(accountId: string): Account | null {
		const row = database
			.query<JsonRow, [string]>("SELECT payload FROM accounts WHERE id = ? AND provider = 'openai'")
			.get(accountId)
	...
	function removeAccount(accountId: string): void {
		const transaction = database.transaction(() => {
			const state = findProviderState('openai')
	...
	function listUsage(): UsageSnapshot[] {
		return queryAll(
			UsageSnapshotSchema,
			"SELECT usage_snapshots.payload FROM usage_snapshots JOIN accounts ON accounts.id = usage_snapshots.account_id WHERE accounts.provider = 'openai' ORDER BY usage_snapshots.account_id"
		)
	}
	...
	function findUsage(accountId: string): UsageSnapshot | null {
		const row = database
			.query<JsonRow, [string]>(
				"SELECT usage_snapshots.payload FROM usage_snapshots JOIN accounts ON accounts.id = usage_snapshots.account_id WHERE usage_snapshots.account_id = ? AND accounts.provider = 'openai'"
			)
			.get(accountId)
	...
	function listProviderStates(): ProviderState[] {
		return queryAll(
			ProviderStateSchema,
			"SELECT payload FROM provider_states WHERE provider = 'openai'"
		)
	}
	...
	function listSwitchRecords(limit = 50): SwitchRecord[] {
		return queryAll(
			SwitchRecordSchema,
			"SELECT payload FROM switch_records WHERE provider = 'openai' ORDER BY created_at DESC LIMIT ?",
			limit
		)
	}
	...
	function tokenAnalytics(
		...
		>(
			"SELECT CAST((at - ?) / ? AS INTEGER) AS b, SUM(input_tokens + output_tokens + cache_read_tokens + cache_creation_tokens) AS tokens FROM token_events WHERE provider = 'openai' AND at >= ? AND at <= ? GROUP BY b"
		)
		const modelQuery = database.query<
			...
			[number, number]
		>(
			"SELECT model, provider, SUM(input_tokens) AS input, SUM(output_tokens) AS output, SUM(cache_read_tokens) AS cached, SUM(cache_creation_tokens) AS cacheCreation FROM token_events WHERE provider = 'openai' AND at >= ? AND at <= ? GROUP BY model, provider"
		)
	...
	function accountTokens(accountId: string, startMillis: number) {
		return database
			.query<
				...
				[string, number]
			>(
				"SELECT model, SUM(input_tokens) AS input, SUM(output_tokens) AS output, SUM(cache_read_tokens) AS cached, SUM(cache_creation_tokens) AS cacheCreation FROM token_events WHERE provider = 'openai' AND account_id = ? AND at >= ? GROUP BY model"
			)
	...
	function tokensBetween(startMillis: number, endMillis: number): number {
		const row = database
			.query<{ tokens: number | null }, [number, number]>(
				"SELECT SUM(input_tokens + output_tokens + cache_read_tokens + cache_creation_tokens) AS tokens FROM token_events WHERE provider = 'openai' AND at >= ? AND at <= ?"
			)
