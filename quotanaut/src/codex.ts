...
export interface PreparedCodexAuthImport {
	account: Account
...
function codexIdentity(auth: CodexAuth): CodexIdentity {
	...
	const accessNamespaced = accessClaims['https://api.openai.com/auth']
	const accountId =
	...
	return {
		...
		plan: namespaced?.chatgpt_plan_type ?? accessNamespaced?.chatgpt_plan_type ?? null,
		userId:
...
export function prepareCodexAuthImport(serialized: string): PreparedCodexAuthImport {
	try {
		const auth = CodexAuthSchema.parse(JSON.parse(serialized))
		const identity = codexIdentity(auth)
		const email = AccountEmailSchema.parse(identity.email)
		...
		return {
			account: {
				...
				profilePath: null,
				provider: 'openai',
				secretReference: `codex:${id}`,
				updatedAt: now
			},
			serializedAuth: JSON.stringify(auth)
		}
	} catch (error) {
		throw new ApplicationError(
			'CODEX_AUTH_INVALID',
			'Codex auth data is invalid or does not contain a verified account identity',
			{ cause: error }
		)
	}
}
...
export async function registerCodexAccount(
	input: { dependencies?: CodexLoginDependencies } = {}
): Promise<PreparedCodexAuthImport> {
	const dependencies = input.dependencies ?? defaultCodexLoginDependencies()
	const temporaryHome = await dependencies.createTemporaryDirectory('quotanaut-register-')
	try {
		...
		return prepareCodexAuthImport(serialized)
	} finally {
...
export async function probeCodex(input: {
	...
	if (account.auth === 'apiKey') {
		await validateOpenAiApiKey(
			await readApiKey(vault, account.secretReference),
			fetchImplementation
		)
		return {
