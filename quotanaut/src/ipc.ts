...
async function dispatch(
	...
	switch (method) {
		...
		case 'account/save': {
			...
			return { account: await manager.saveAccount(parsed) }
		}
		case 'account/remove': {
								...
								code:
									error instanceof ApplicationError
										...
										: error instanceof z.ZodError
											? 'INVALID_REQUEST'
											: 'INTERNAL_ERROR',
								message: error instanceof z.ZodError ? z.prettifyError(error) : errorMessage(error)
							},
							id: parsed.data.id
						})
					}
...
export function requestAccountSave(socketPath: string, serializedAuth: string): Promise<Account> {
	return managerRequest({
		method: 'account/save',
		params: { serializedAuth },
		schema: z.object({ account: AccountSchema }),
		socketPath,
		timeoutMilliseconds: 15_000
