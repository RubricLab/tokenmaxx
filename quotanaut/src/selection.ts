...
export function selectRotation(input: RotationInput): RotationDecision {
	...
	if (activePressure === null && !activeSnapshot.hardLimitReached) {
		return { reason: 'activeUsageUnknown', rotate: false }
	}
	const effectiveActivePressure = activePressure ?? 100
	if (!activeSnapshot.hardLimitReached && effectiveActivePressure < policy.thresholdPercent) {
	...
	return {
		...
		sourcePressure: effectiveActivePressure,
