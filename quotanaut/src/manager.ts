...
import { type ApplicationPaths, ensureProxyToken } from './paths.ts'
...
import { type CredentialVault, exclusive, exclusiveReferences } from './vault.ts'
...
export class AccountManager {
		...
		return this.withProviderOperation(provider, async () => {
			...
			try {
				...
				throw new ApplicationError(
					...
					`${account.label} needs re-login — run: quotanaut login`,
					{ cause }
				)
			}
		})
	...
	public async start(): Promise<void> {
		this.#stopping = false
		const clientToken = await ensureProxyToken(this.#paths)
	...
	public async saveAccount(input: { serializedAuth: string }): Promise<Account> {
		const prepared = prepareCodexAuthImport(input.serializedAuth)
			...
			const previous = this.#store
				...
				.find(
					candidate =>
						...
						candidate.externalUserId === prepared.account.externalUserId
				)
			const account =
				previous === undefined
					? prepared.account
					: {
							...
							id: previous.id,
							onThreshold: previous.onThreshold,
							secretReference: `codex:${previous.id}`
						}
			return exclusiveReferences(
				[account.secretReference, previous?.secretReference ?? account.secretReference],
				() => this.savePreparedAccount(account, prepared.serializedAuth, previous ?? null)
			)
		})
	}

	private async savePreparedAccount(
		account: Account,
		serializedAuth: string,
		previous: Account | null
	): Promise<Account> {
		const previousCredential =
			previous === null ? null : await this.#vault.read(previous.secretReference)
		let saved = false
		try {
			await this.#vault.write(account.secretReference, serializedAuth)
			this.#store.saveAccount(account)
			saved = true
			if (previous !== null && previous.secretReference !== account.secretReference) {
				await this.#vault.remove(previous.secretReference)
			}
			const state = this.#store.findProviderState(account.provider)
			...
			try {
				if (saved) {
					if (previous === null) {
						...
						this.#store.saveAccount(previous)
					}
				}
				if (previous !== null && previousCredential !== null) {
					await this.#vault.write(previous.secretReference, previousCredential)
				}
				if (previous?.secretReference !== account.secretReference || previousCredential === null) {
					await this.#vault.remove(account.secretReference)
				}
			} catch (rollbackError) {
				throw new ApplicationError(
					'ACCOUNT_SAVE_ROLLBACK_FAILED',
					'Could not roll back a failed account credential save safely',
	...
	public async refreshAccount(account: Account): Promise<void> {
		return this.withProviderOperation(account.provider, async () => {
			...
			if (current === null || current.provider !== account.provider) {
				return
			}
			await this.probeAndSave(current)
		})
	}
	...
	private async probeAndSave(account: Account): Promise<void> {
		...
		const result = await probeCodex({ account, ...shared })
	...
	private codexAccountOrThrow(accountId: string): Account {
	...
	public async removeAccount(accountId: string): Promise<void> {
		return this.withProviderOperation('openai', async () => {
			...
			return exclusive(account.secretReference, async () => {
				const credential = await this.#vault.read(account.secretReference)
				await this.#vault.remove(account.secretReference)
				try {
					this.#store.removeAccount(account.id)
				} catch (error) {
					if (credential !== null) {
						try {
							await this.#vault.write(account.secretReference, credential)
						} catch (rollbackError) {
							throw new ApplicationError(
								'ACCOUNT_REMOVE_ROLLBACK_FAILED',
								'Could not roll back a failed account removal safely',
								{ cause: new AggregateError([error, rollbackError]) }
							)
						}
					}
	...
	private async performRefreshAll(): Promise<void> {
		for (const listed of this.#store.listAccounts()) {
			if (this.#stopping) {
				return
			}
			const account = this.#store.findAccount(listed.id)
			if (account === null) {
				continue
			}
			if (!account.enabled) {
		...
		if (!this.#stopping) {
			await this.evaluateAutomation('openai')
		}
	}
	...
	public async noteRateLimitObservation(event: ProxyLimitEvent): Promise<void> {
		...
		if (event.observation.limited) {
			await this.withProviderOperation(event.provider, async () => {
				if (!this.saveRateLimitObservation(event)) {
					return
				}
				this.#observationEvaluatedAt.set(event.provider, event.at)
				await this.evaluateAutomation(event.provider)
			})
			return
		}
		if (!this.saveRateLimitObservation(event)) {
		...
		if (event.at - lastEvaluated >= 5_000) {
			this.#observationEvaluatedAt.set(event.provider, event.at)
	...
	private saveRateLimitObservation(event: ProxyLimitEvent): boolean {
