...
function responseSignature(response: UsageResponse): string {
	...
	return windows
		.map(
			([id, window]) => `${id}:${window?.reset_at ?? 'none'}:${window?.limit_window_seconds ?? 'none'}`
		)
		.sort()
		.join('|')
}
...
function consensusResponses(responses: readonly UsageResponse[]): UsageResponse[] {
	...
	return majority !== undefined && majority.length > responses.length / 2 ? majority : [...responses]
}
...
export async function fetchCodexUsage(input: {
	...
	const bodies = await Promise.all(
		Array.from({ length: sampleCount }, () =>
			fetchUsageResponse({ credential: input.credential, fetchImplementation, identity })
		)
	)
